(() => {
	...
	/* Reveal on scroll — Framer motionViewport once + amount 0.16 on brand pages */
	const revealNodes = document.querySelectorAll("[data-reveal]");
	if (revealNodes.length) {
		if (reduceMotion || !("IntersectionObserver" in window)) {
			revealNodes.forEach((node) => node.classList.add("is-visible"));
		} else {
			const isBrandPage = Boolean(document.querySelector(".brand-page"));
			const observer = new IntersectionObserver(
				(entries) => {
					entries.forEach((entry) => {
						if (!entry.isIntersecting) return;
						const node = entry.target;
						const delayAttr = node.getAttribute("data-reveal-delay");
						if (delayAttr !== null && delayAttr !== "") {
							...
							const stagger = Number(node.getAttribute("data-reveal-stagger") || "");
							if (Number.isFinite(stagger) && stagger > 0) {
								/* Framer gallery: delay index * 0.08s */
								node.style.transitionDelay = `${stagger * 0.08}s`;
							}
						}
				...
				{
					threshold: 0.16,
					rootMargin: isBrandPage ? "0px" : "0px 0px -8% 0px",
				}
			);
...
(() => {
	...
	if (stickyBar) {
		...
		const reduceStickyMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
		let lastY = window.scrollY;
		let visible = false;
		let dismissed = false;
		let leaveTimer = 0;
		...
		const setVisible = (next) => {
			if (dismissed || next === visible) return;
			visible = next;
			window.clearTimeout(leaveTimer);
			if (visible) {
				stickyBar.hidden = false;
				stickyBar.classList.remove("is-leaving");
				requestAnimationFrame(() => stickyBar.classList.add("is-visible"));
			} else {
				stickyBar.classList.remove("is-visible");
				stickyBar.classList.add("is-leaving");
				leaveTimer = window.setTimeout(() => {
					stickyBar.classList.remove("is-leaving");
					stickyBar.hidden = true;
				}, reduceStickyMotion ? 10 : 480);
			}
		};
		...
		stickyBar.querySelector("[data-concierge-dismiss]")?.addEventListener("click", () => {
			dismissed = true;
			setVisible(false);
			window.setTimeout(() => {
				stickyBar.remove();
			}, reduceStickyMotion ? 10 : 500);
		});
...
(() => {
	...
	/* Keukens overview hero crossfade */
	const keukensHero = document.querySelector("[data-keukens-hero]");
	const keukensHeroImg = keukensHero?.querySelector("[data-keukens-hero-image]");
	const keukensHeroJson = document.getElementById("kc-keukens-hero-images");
	if (keukensHero && keukensHeroImg && keukensHeroJson && !reduceMotion) {
		try {
			const images = JSON.parse(keukensHeroJson.textContent || "[]");
			if (Array.isArray(images) && images.length > 1) {
				let index = 0;
				window.setInterval(() => {
					index = (index + 1) % images.length;
					keukensHeroImg.style.transition = "opacity 1.2s ease";
					keukensHeroImg.style.opacity = "0";
					window.setTimeout(() => {
						keukensHeroImg.src = images[index];
						keukensHeroImg.style.opacity = "1";
					}, 400);
				}, 6000);
			}
		} catch (_error) {
	...
	/* Brand page hero parallax (React BrandPage scroll transform) */
	const brandHero = document.querySelector("[data-brand-hero]");
	const brandHeroMedia = brandHero?.querySelector("[data-brand-hero-parallax]");
	if (brandHero && brandHeroMedia && !reduceMotion) {
		const onBrandScroll = () => {
			const rect = brandHero.getBoundingClientRect();
			const height = Math.max(rect.height, 1);
			const progress = Math.min(1, Math.max(0, -rect.top / height));
			brandHeroMedia.style.transform = `translate3d(0, ${progress * 24}%, 0)`;
			brandHeroMedia.style.opacity = String(1 - progress * 0.7);
		};
		onBrandScroll();
		window.addEventListener("scroll", onBrandScroll, { passive: true });
	}

	/* Brand FAQ — React Accordion type="single" collapsible parity */
	document.querySelectorAll("[data-brand-faq]").forEach((root) => {
		const items = [...root.querySelectorAll(".brand-faq__item")];
		const setOpen = (target, open) => {
			const trigger = target.querySelector(".brand-faq__trigger");
			const panel = target.querySelector(".brand-faq__panel, .brand-faq__content");
			if (!trigger || !panel) return;
			target.setAttribute("data-state", open ? "open" : "closed");
		...
		items.forEach((item) => {
			const trigger = item.querySelector("button.brand-faq__trigger");
			if (!trigger) return;
			setOpen(item, false);
